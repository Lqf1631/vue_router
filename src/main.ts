...
// 导入全局插件 loadingBar 的 loading loaded 方法
import { loading, loaded } from './components/loadingBar/index'
...
router.beforeEach((to, from, next) => {
  ...
  if (publicRoute.includes(to.path) || localStorage.getItem('token')) {
    // 开始跳转时，调用全局插件 loadingBar 的 loaded ，加载条开始移动
    loaded()
    next()
  }
  // 反之则跳转到 login 界面
...
router.afterEach((to,from)=>{
  // 跳转完成时，调用全局插件 loadingBar 的 loaded ，完成加载条
  loading()
})